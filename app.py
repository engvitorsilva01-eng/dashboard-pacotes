...
# =========================
# Hide Streamlit chrome (mais "produto")
# =========================
st.markdown(
    """
<style>
...
# =========================
# CSS (mobile + profissional + didático)
# =========================
st.markdown(
    """
<style>
...
.caption { opacity: 0.80; font-size: 0.95rem; margin-top: 0.15rem; margin-bottom: 0.9rem; }
...
.badge-warn{ border-color: rgba(255, 180, 60, 0.72); }
.badge-ok{ border-color: rgba(110, 255, 180, 0.35); }

.small{ font-size: 0.90rem; opacity: 0.86; margin-top: 6px; line-height: 1.35; }
hr{ opacity: 0.12; }
</style>
""",
    unsafe_allow_html=True,
...
def render_card(pacote, codigo, status, dias, data_envio, data_receb):
    if status == "Recebido":
        badge_class = "badge badge-ok"
        badge_text = "✅ Recebido"
        drec = "—"
        if pd.notna(data_receb):
            drec = data_receb.strftime("%d/%m/%Y") if hasattr(data_receb, "strftime") else str(data_receb)
        line = f"📅 Recebido em: <b>{drec}</b>"
    else:
...
<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
  <div>
    ...
    <div style="opacity:.78;margin-top:4px;">Status + dias em espera. Clique e rastreie nos Correios.</div>
  </div>
</div>
""",
    unsafe_allow_html=True,
...
try:
    ...
    st.error(f"Não encontrei **{ARQUIVO_EXCEL}** na raiz do projeto.")
    st.info("✅ Coloque a planilha na mesma pasta do app.py.")
    st.stop()
...
with st.expander("ℹ️ Entenda em 10 segundos", expanded=False):
    ...
    st.write("• **Dias em espera**: desde a data de envio (se faltar envio, usa data do pedido).")
...
q = st.text_input(
    "Digite o Pacote ou Código de Rastreio",
    placeholder="Ex: 35 ou LZ373185768CN",
...
st.caption("Aqui você vê o que está mais atrasado.")
...
if em_df.empty:
    ...
    for _, r in em_df.iterrows():
        ...
        render_card(pacote, codigo, status, dias, d_envio, d_rec)
